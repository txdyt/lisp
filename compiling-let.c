...
enum {
  ...
  kHeapPtrMake = ~kHeapTagMask, // 0b1111...1000
};
...
uword Object_address(void *obj) { return (uword)obj & kHeapPtrMake; }
...
ASTNode *AST_new_char(word value) {
...
int node_to_str(ASTNode *node, char *buf, word size) {
  ...
  if (AST_is_pair(node)) {
    ...
    result += snprintf(buf + result, size, "(");
    result += node_to_str(AST_pair_car(node), buf + result, size);
    result += node_to_str(AST_pair_cdr(node), buf + result, size);
    return result;
  }
...
WARN_UNUSED int Compile_let(Buffer *buf, ASTNode *bindings, ASTNode *body,
  ...
  Emit_store_reg_indirect(buf, /*dst=*/Ind(kRbp, stack_index), /*src=*/kRax);
  // Bind the name
...
#define ASSERT_IS_INT_EQ(node, val)                                            \
  do {                                                                         \
    ASTNode *__tmp = node;                                                     \
    if (AST_is_error(__tmp)) {                                                 \
      fprintf(stderr, "Expected a int but got an error.\n");                   \
    }                                                                          \
...
TEST compile_unary_integer_to_char(Buffer *buf) {
  ...
  byte expected[] = {0x48, 0xc7, 0xc0, 0x84, 0x01, 0x00, 0x00, 0x48,
                     0xc1, 0xe0, 0x06, 0x48, 0x83, 0xc8, 0xf};
  EXPECT_FUNCTION_CONTAINS_CODE(buf, expected);
  Buffer_make_executable(buf);
  uword result = Testing_execute_expr(buf);
